...
/*
  ========================================
  Default active nav button when page loaded
  ========================================
*/
...
for (let j = 0; j < $('.alr').length; j++) {

	for (let i = 0; i < $(`.alr:eq(${j}) .picWord`).length; i++) {

		if (i == 0) {
			$(`.alr:eq(${j}) .picWord:eq(${i})`).mouseover(function(){
				mouseOverChangeColor(i,j,c1);
				mouseOverChangeColor(i+1,j,c2);
				mouseOverChangeColor(i+2,j,c3);
				mouseOverChangeColor(i+3,j,c4);
			});
			$(`.alr:eq(${j}) .picWord:eq(${i})`).mouseleave(function(){
				mouseLeaveChangeColor(i,j);
				mouseLeaveChangeColor(i+1,j);
				mouseLeaveChangeColor(i+2,j);
				mouseLeaveChangeColor(i+3,j);
			});
...
function mouseOverChangeColor (i, j, color){
	$(`.alr:eq(${j}) .picWord:eq(${i})`).css('opacity','0.9');
	$(`.alr:eq(${j}) .picWord:eq(${i})`).css('background-color', color);
}

function mouseLeaveChangeColor (i, j){
	$(`.alr:eq(${j}) .picWord:eq(${i})`).css('opacity','0');
}
...
(function (root, factory) {
	 ...
	 * @param {Boolean} clip         If true, adjust scroll distance to prevent alrupt stops near the bottom of the page
	 * @returns {Number}
	 */
